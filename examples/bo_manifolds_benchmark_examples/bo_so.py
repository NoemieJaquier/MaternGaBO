...
def get_bo_attributes(manifold_name, kernel_name, acquisition_name, test_function_name):
    ...
    elif hasattr(kernel_so, kernel_name):
        kernel = getattr(kernel_so, kernel_name)
    else:
...
def get_bounds(dimension):
    bounds = torch.stack([-torch.ones(dimension ** 2, dtype=torch.float64),
                          torch.ones(dimension ** 2, dtype=torch.float64)])
    return bounds
...
def get_constraints(bo_type):
    if bo_type == "GaBO":
        ...
        def orthogonal_constraint(x):
            dimension = int(np.sqrt(x.shape[0]))
            x_mat = x.reshape((dimension, dimension))
            return np.sum(np.dot(x_mat, x_mat.T) - np.eye(dimension))

        def det1_constraint(x):
            dimension = int(np.sqrt(x.shape[0]))
            x_mat = x.reshape((dimension, dimension))
            return np.linalg.det(x_mat) - 1.

        return [{'type': 'eq', 'fun': orthogonal_constraint}, {'type': 'eq', 'fun': det1_constraint}]
...
def get_preprocessing(bo_type):
    if bo_type == "GaBO":
        def process(x):
            ...
            shape = list(x.shape[:-1]) + [dimension, dimension]
            return x.view(shape)
...
def get_postprocessing(bo_type, manifold):
    if bo_type == "GaBO":
        def process(x):
            shape = list(x.shape[:-2]) + [x.shape[-1] * x.shape[-2]]
            return x.view(shape)
        ...
        def post_processing_init(x):
            nb_data = np.prod(x.shape[:-1])
            samples = np.array([manifold.rand().reshape(-1) for n in range(nb_data)])
            return torch.from_numpy(samples).view(x.shape)
...
def main(manifold_name, dimension, kernel_name, acquisition_name, bo_type, test_function_name, nb_iter_bo, seed_id,
    ...
    x_init = np.array([x_init[i].reshape(dimension ** 2) for i in range(nb_data_init)])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--benchmark", dest="benchmark_function", default="Ackley",
                        help="Set the benchmark function. Options: Ackley, Rosenbrock, StyblinskiTang, Levy, "
                             "ProductOfSines, ...")
    parser.add_argument("--kernel", dest="kernel", default="SORiemannianGaussianKernel",
                        help="Set the kernel. Options: RBFKernel, MaternKernel "
                             "SORiemannianGaussianKernel, SORiemannianMaternKernel")
    ...
    parser.add_argument("--save", dest="save", type=bool, default=False,
                        help="Save or not the log of the BO run?")
    ...
    # Manifold
    manifold_name = "SpecialOrthogonalGroup"
