...
class ConstrainedTrustRegions(Solver):
    ...
    def solve(self, problem, x=None, eq_constraints=None, ineq_constraints=None, mininner=1, maxinner=None,
        ...
        # Decorate constraints function
        eq_constraints = [pymanopt.function.PyTorch(con_function) for con_function in eq_constraints]
        ineq_constraints = [pymanopt.function.PyTorch(con_function) for con_function in ineq_constraints]

        neq_cons = len(eq_constraints)
        nineq_cons = len(ineq_constraints)

        # Create problems for constraints
        eq_con_probs = [Problem(problem.manifold, eq_constraints[i], verbosity=0) for i in range(neq_cons)]
        ineq_con_probs = [Problem(problem.manifold, ineq_constraints[i], verbosity=0) for i in range(nineq_cons)]
        ...
        while True:
            ...
            # Compute constraints functions and gradients
                ...
                if const_inner > Delta_cons ** 2:
                    ...
                    if nineq_cons > 0:
                        idx = np.hstack((np.array(range(0, neq_cons), dtype=int),
                                         np.where(const_term[neq_cons:] < 0)[0] + neq_cons))
                    else:
                    ...
                    if discriminant >= 0.:
                        # Check against too small denominator
                        if qeq_a > 1e-16:
