...
def gen_candidates_scipy(
    ...
    def f(x):
        X = (torch.from_numpy(x).to(initial_conditions).contiguous().requires_grad_(True)).to(device)
    ...
    for i in range(x0.shape[0]):
        res = minimize(f, x0[i, 0].cpu().detach().numpy(), method="SLSQP", jac=True, bounds=bounds, constraints=constraints,
